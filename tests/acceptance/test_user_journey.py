...
async def test_public_user_journey() -> None:
    ...
    assert "26.08.2026 (Chorshanba)\nToshkent shahri" in harness.last_message(9)
    ...
    assert "19:07 — Shom (+4)" in harness.last_message(9)
    ...
    assert "26.08.2026 (Chorshanba)\nSamarqand shahri" in harness.last_message(9)
    assert "(+4)" not in harness.last_message(9)

    harness.clear_messages()
    second_daily = await harness.run_daily_job("2026-08-27T21:00:00+05:00")
    assert second_daily.sent == 1
    assert "28.08.2026 (Juma)\nSamarqand shahri" in harness.last_message(9)
    assert "(+4)" not in harness.last_message(9)
